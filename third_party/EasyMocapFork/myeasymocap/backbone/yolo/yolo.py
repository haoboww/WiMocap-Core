...
class BaseYOLOv5:
    ...
    # def __init__(self, ckpt='/home/bupt630/Dabai/AmmWave/EasyMocap/yolov5m.pt', model='yolov5m', name='object2d', multiview=True) -> None:
        # Prefer the vendored repository so the portable package works offline.
        vendored_repo = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', '..', '..', 'vendor', 'yolov5')
        )
        cached_repo = os.path.expanduser('~/.cache/torch/hub/ultralytics_yolov5_master')
        local_repo = vendored_repo if os.path.exists(vendored_repo) else cached_repo
    ...
    def __call__(self, images, imgnames): # 这里好像默认是多视角了，需要继承一下单视角的
        ...
        for nv in range(len(images)):
            ...
            select = select.astype(np.float32)
            # TODO: add track here
