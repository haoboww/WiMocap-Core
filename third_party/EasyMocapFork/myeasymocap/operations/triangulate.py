...
def batch_triangulate(keypoints_, Pall, min_view=2):
    ...
    if valid_joint.size == 0:
        return np.zeros((keypoints_.shape[1], 4), dtype=np.float32)
...
class SimpleTriangulate:
    ...
    def __call__(self, keypoints, cameras):
        ...
        if np.count_nonzero(keypoints[..., -1] > 0) == 0:
            return {'keypoints3d': np.zeros((keypoints.shape[1], 4), dtype=np.float32)}
        if self.mode == 'naive':
